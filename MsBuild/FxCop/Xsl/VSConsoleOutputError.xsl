...
<xsl:template match="Issue">
	<xsl:if test="@Path"><xsl:value-of select="@Path"/>\<xsl:value-of select="@File"/>(<xsl:value-of select="@Line"/>,1) : </xsl:if><xsl:if test="not(@Path)"><xsl:value-of select="/FxCopReport/Localized/String[@Key='LocationNotStoredInPdb']/text()" /> : </xsl:if><xsl:if test="../@BreaksBuild">error </xsl:if><xsl:if test="not(../@BreaksBuild)">error </xsl:if> : <xsl:apply-templates select=".." mode="parentMessage" /> : <xsl:value-of select="text()" /><xsl:text disable-output-escaping="yes">&#xD;&#xA;</xsl:text>
</xsl:template>
